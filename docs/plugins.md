...
One thing a mount does **not** cover by default: a registry or struct mounted at a prefix answers for that whole prefix, misses included. If a mounted struct sits at `/instrument` and a plugin claims the same root, the struct wins every path under it and the plugin is never reached. `Router::with_mount_fallthrough()` sends a mount's *misses* to the fallback, which is what a plugin table behind a root-mounted object needs — and is the only thing that makes the fallback reachable at all when the object is mounted at `""`. A path the struct actually serves still belongs to the struct, so genuinely colliding roots still want separate names.
...
- **`Plugin::load_origin()` says which of the two happened.** `LoadOrigin::Mapped` when the load read the file, `AlreadyResident` when it reached a copy already loaded — the one deployment where the distinction matters is the one where someone rebuilt in place, and where the path is not the host's to choose. `AlreadyResident` is not an error: the handle works, the plugin works, and only the *load* was a no-op. A hot-reload endpoint that reports success either way will be believed; this is what lets it report the truth instead. There is a third answer, `Unknown`, on the few unix targets whose loader has no `RTLD_NOLOAD` to probe with (OpenBSD, Haiku, AIX, VxWorks) — report it as "cannot tell" rather than folding it into either. Do not read `REPE_ERROR_ALREADY_INITIALIZED` as the same signal — a plugin that initializes lazily returns it on a genuine first load.
