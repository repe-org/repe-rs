...
Status: **implemented** behind the `value-stream` feature (`src/value_stream.rs`). SVS is a **pull-only** contract, so this is the whole of it on the wire, pulled over the sync `Client`, the async `AsyncClient`, or (with the `websocket` feature) a `WebSocketClient` (see below). Companion to [Streaming with Backpressure](streaming.md); this is the bulk-transfer shape for *serialized, compressed, unknown-length* payloads.
...
let (tx, rx) = sync_channel::<Msg>(SESSION_DEPTH);       // bounded: Chunk(bytes) + a terminal End/Fail marker — backpressure lives here
...
// On `next(stream_id)`: rx.recv() -> response body; an explicit End marker sets last = true. A bare channel close (producer thread vanished) is treated as a Fail, never a silently-short clean end, so a truncated production cannot masquerade as `last`.
...
- **Content integrity that keeps a standard `format` tag.** Transport already checksums each frame and atomic rename (§3.2) already rejects a *truncated* file, but neither verifies a completed transfer against a producer-committed content digest. App-framed single-pass integrity **ships today** via the `with_writer_stream` producer: the closure owns the `Write` sink, so it serializes the body through a hashing tee and appends the digest in the one streaming pass, emitting `payload || digest` tagged `format = RawBinary`; the consumer lifts the trailer off and verifies it in one pass with `pull_to_file_trailer_verified` (async: `pull_to_file_trailer_verified_async`), which holds the last N bytes back, hashes only the payload prefix, and commits the payload — trailer stripped — only on a match. (For full manual control it can instead do a plain pull, `pull_to_file` / `pull_consume` / `pull_consume_async`, and split the trailer by hand; not `pull_to_file_verified_async`, which would hash the trailer too and leave it on disk.) The digest covers the **logical** (pre-compression) bytes; `opts.compression` is transparent end to end. What remains **deferred** is single-pass integrity that *also* keeps a standard `format` tag (BEVE/JSON/UTF-8): appending a trailer to a standard-tagged value breaks a clean deserialize, and folding a trailer *into* the stream would break §3's byte-exact reassembly — so it needs a separate pulled `/_svs/digest` route, held until a second SVS implementation exists to conform against and a producer is confirmed CPU-bound on the double-encode pass while needing a standard tag.
