//! End-to-end coverage for the bulk numeric fast path: `Router::with_typed_slice`
//! paired with `AsyncClient::call_typed_slice`, plus wire-interop with the serde
...
/// return its address, for exercising the blocking `Client::call_typed_slice` path.
...
async fn call_typed_slice_roundtrips_f64_and_f32() {
    let router = Router::new()
        .with_typed_slice::<f64, f64, _>("/scale2", |xs| Ok(xs.iter().map(|x| x * 2.0).collect()))
        .with_typed_slice::<f32, f32, _>("/negate", |xs| Ok(xs.iter().map(|x| -x).collect()));
    ...
    let out2: Vec<f64> = client
        .call_typed_slice_with_timeout("/scale2", &xs2, Duration::from_secs(5))
        .await
        .expect("call_typed_slice_with_timeout");
    assert_eq!(out2, vec![2.0, 4.0, 6.0]);
}

#[test]
fn sync_client_call_typed_slice_roundtrips() {
    // The blocking `Client::call_typed_slice` path against a synchronous `Server`.
    let router = Router::new()
        .with_typed_slice::<f64, f64, _>("/scale2", |xs| Ok(xs.iter().map(|x| x * 2.0).collect()));
    ...
    let out: Vec<f64> = client
        .call_typed_slice("/scale2", &xs)
        .expect("sync call_typed_slice");
    assert_eq!(out, xs.iter().map(|x| x * 2.0).collect::<Vec<_>>());

    let out2: Vec<f64> = client
        .call_typed_slice_with_timeout("/scale2", &xs, Duration::from_secs(5))
        .expect("sync call_typed_slice_with_timeout");
    assert_eq!(out2, xs.iter().map(|x| x * 2.0).collect::<Vec<_>>());
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn slice_route_interops_with_serde_client() {
    // A with_typed_slice route decodes a body sent through the serde path
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn serde_route_interops_with_typed_slice_client() {
    // The mirror: a serde `with_typed` route decodes a body sent through the bulk
    // `call_typed_slice` path, and the bulk client decodes the serde-framed response.
