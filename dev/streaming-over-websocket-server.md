...
Proposal. Design decisions resolved (see [Resolved design decisions](#resolved-design-decisions)); not yet implemented. Written against repe 2.5.1.
...
Inside the blocking task it reconstructs a `CallContext` from the owned peer and path and runs **the same route resolution and dispatch the inline path uses** — version/query-format validation, notify suppression, method-not-found, and handler-error→error-response must be identical whether a path is inline or off-reader. Concretely, factor the body of `route_request_with_ctx` into a resolve step (returns the `Arc<dyn HandlerErased>`, or an early response for version/format errors) and a dispatch step, so the blocking task calls the dispatch step rather than invoking the handler raw. For a request that yields a response it sends it with `outbound_tx.blocking_send(resp)` so the writer task delivers it. `spawn_blocking` (not `tokio::spawn`) is correct here: handlers are synchronous and a `repe::stream` producer blocks on a `Condvar`, so it must not occupy a runtime worker.
...
Off-reader dispatch is capped per connection (see [decision 3](#resolved-design-decisions)). `spawn_blocking` draws from tokio's process-global blocking pool (512 threads by default), and a producer parked in `wait_for_credit` holds its thread for the whole transfer, so an unbounded off-reader path lets a few connections starve every other blocking task in the process. A per-connection semaphore (`with_offreader_limit(n)`, finite default) bounds it. On saturation the reader must **not** block waiting for a slot — that would stop it draining the ACK/cancel frames the in-flight transfers need to finish and free a slot, recreating the very deadlock this feature removes — so it `try`-acquires and, on failure, synthesizes an error response (an application error at/above `ErrorCode::ApplicationErrorBase`, since REPE defines no protocol-level "busy" code) and keeps reading.
...
## Doc cleanup (while in `repe::stream`)

- The `repe::stream` module doc closes with a reference to a downstream consumer by name (`src/stream.rs:23`). A published crate should not name downstream projects; genericize it (for example, "see your transport's sink for a concrete embedding").
- `docs/streaming.md`'s sketch is broken three independent ways against the current `repe::stream` API; a reader who copies it will not compile. Reconcile all three in the same pass:
  - `push_replay(chunk_offset, false, body.clone())` passes three arguments; the signature is `push_replay(offset, data_len, last, body_bytes)` (four — the logical `data_len` is missing).
  - it matches `ReconnectOutcome::ResumeReady =>` as a unit variant, but the variant carries a payload: `ReconnectOutcome::ResumeReady(PendingResume)`.
  - it calls `control.take_pending_resume()`, which does not exist. The staged resume is consumed inside `wait_for_reconnect` and handed back through `ResumeReady(pending)`; the sketch should bind `pending` from the match arm, not call a separate taker.
